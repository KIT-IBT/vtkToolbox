...
function writeVTK(inStruct, filename, verbose, dataMode)

if nargin < 4
    if nargin < 3
        vtkWrite(inStruct, filename, false);
    else
        vtkWrite(inStruct, filename, verbose);
    end
else
    vtkWrite(inStruct, filename, verbose, dataMode);
end
